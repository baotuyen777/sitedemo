	...
	<?php echo get_the_category_list( __( ', ', 'flatsome' ) ) ?>
		...
		<?php
			...
			elseif ( is_author() ) :
				...
				*/
				the_post();
				printf( __( 'Bài viết của: %s', 'flatsome' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' );
			...
			elseif ( is_day() ) :
				printf( __( 'Lưu trữ ngày: %s', 'flatsome' ), '<span>' . get_the_date() . '</span>' );

			elseif ( is_month() ) :
				printf( __( 'bài viết trong tháng: %s', 'flatsome' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );

			elseif ( is_year() ) :
				printf( __( 'Bài viết năm: %s', 'flatsome' ), '<span>' . get_the_date( 'Y' ) . '</span>' );
