 ...
 * Plugin URI: http://www.laivanduc.com/store/plugin-dat-hang-nhanh-cho-wordpress.html
 * Description: This is a plugin that allows us to checkout woo Ajax functionality in WordPress
 * Version: 1.3.1
 * Author: Lai van duc
 * Author URI: http://www.laivanduc.com/store/plugin-dat-hang-nhanh-cho-wordpress.html
 * License: GPL2
 */
...
function muahangnhanh_activated(){
    ...
    global $wpdb;
    $str = "SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'wp_muahangnhanh' AND COLUMN_NAME = 'addtion'";
    $res = $wpdb->get_row($str);
    if($res == NULL) {
        $str = "ALTER TABLE `wp_muahangnhanh` ADD `addtion` TEXT AFTER `thanhtien`";
        $res = $wpdb->get_row($str);
    }
}
...
function formmail(){
    global $product;
?>
    <form id="muahangnhanh" method="post" action="">
   ...
   <table class="shop_attributes">
	...
	<?php endif; $attributes = $product->get_attributes() ?>

	<?php foreach ( $attributes as $attribute ) : ?>
		<tr>
			<th><?php echo wc_attribute_label( $attribute->get_name() ); ?></th>
			<td><select name="<?php echo wc_attribute_label( $attribute->get_name() ) ?>"><?php
				$values = array();
				...
				foreach($attribute_values as $key){
...
function muahangnhanh() {
    ...
    echo "<a data-popup-open='muahangnhanh' href='#'><div class='detailcall-1'><span>ĐẶT HÀNG NHANH</span></div></a>
